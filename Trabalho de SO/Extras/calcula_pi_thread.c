...
int main (){
	pthread_t thread_idA;
 	...
 	//Cria Thread A
 	rstatus = pthread_create (&thread_idA, NULL, gauss, &i);
 	if(rstatus != 0){
 	...
 	i++;
 	//Cria Thread B
 	rstatus = pthread_create (&thread_idB, NULL, borwein, &i);
 	if(rstatus != 0){
 	...
 	i++;
 	//Cria Thread C
 	rstatus = pthread_create (&thread_idC, NULL, monte_carlo, &i);
 	if(rstatus != 0){
